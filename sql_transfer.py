...
try:
    for patron_oid, count, hold_status, fee_status, _, hold_remove_time, invoice_oid, registrar_hold in db.all("SELECT * FROM overdues_old"):
        ...
        o_stime = datetime.now()
        for alloc in allocs:
            overdue = False
            if alloc['realEndTime']:
                r_end = datetime.strptime(alloc['realEndTime'], '%Y-%m-%dT%H:%M:%S.%f%z')
            else:
                r_end = datetime.strptime("2024-06-07T10:25:00.000000-05:00", '%Y-%m-%dT%H:%M:%S.%f%z')
        ...
        if invoice_oid and hold_remove_time and hold_remove_time > datetime.now():
            ...
            hold_len = hold_len.days + (hold_len.seconds / 60 / 60 / 24)
            ...
            invoices = wco.get_patron(patron_oid, ['invoices']).json()['payload']['invoices']
            if invoices:
                invoice_oid = invoices[-1]['oid']
            else:
                invoice_oid = -1
            hold_len = 0
            hold_rtime = None
        ...
        db.run("INSERT INTO overdues VALUES (%(p_oid)s, %(count)s, %(hold_count)s, %(fee_count)s, CAST('%(hold_len)sD' AS INTERVAL), %(hold_rtime)s, %(i_id)s, %(reg_hold_count)s)",
                p_oid = patron_oid,
                count = count,
        ...
        db.run("INSERT INTO invoices VALUES (%(i_id)s, %(count)s, %(h_status)s, %(f_status)s, %(reg_hold)s, CAST('%(hold_len)s' AS INTERVAL), %(o_stime)s, %(hold_rtime)s, %(ck_id)s, %(p_id)s, %(waived)s, %(exp)s)",
                i_id = invoice_oid,
                count = count,
